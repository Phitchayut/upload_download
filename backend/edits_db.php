...
if (isset($_POST['update'])) {
    ...
    if (is_uploaded_file($_FILES['imgInput']['tmp_name'])) {
        $new_image_name = 'header' . uniqid() . "." . pathinfo(basename($_FILES['imgInput']['name']), PATHINFO_EXTENSION);
        $image_upload_path = "../frontend/upload/header/" . $new_image_name;
        move_uploaded_file($_FILES['imgInput']['tmp_name'], $image_upload_path);
    } else {
        $new_image_name = "$old_header_img";
    }
    if ($new_image_name == '') {
        $new_image_name = "No Header!";
    }

    $stmt = $conn->prepare("UPDATE tbl_upload SET doc_name = :doc_name, doc_file = :doc_file, header_img = :header_img WHERE id = :id");
